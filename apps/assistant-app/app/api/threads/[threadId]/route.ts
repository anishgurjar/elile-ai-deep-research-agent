...
import { threadIdParam, updateThreadBody } from "../../_middleware/schemas";
...
export async function GET(_req: NextRequest, { params }: RouteParams) {
  try {
    ...
    const thread = await client.getThread(parsedId.data);
...
export async function PATCH(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    const client = createLangGraphClient(authResult.token);
    const thread = await client.updateThread(parsedId.data, parsed.data);
    return NextResponse.json(thread);
  } catch (e: unknown) {
    const error = e as { message?: string; status?: number };
    return NextResponse.json(
