...
describe("POST /api/threads/[threadId]/runs/wait", () => {
  it("should return 401 when not authenticated", async () => {
    mockAuth.mockResolvedValue({ userId: null, getToken: vi.fn() } as never);

    const req = new NextRequest("http://localhost/api/threads/x/runs/wait", {
      method: "POST",
  ...
  it("should proxy valid wait request", async () => {
